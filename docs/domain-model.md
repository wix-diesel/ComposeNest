...
外部変更からの管理再開は、アーキテクチャ設計で「確認後の退避と保存設定からの復帰」を具体化し、ユーザー採用済み。Engine再登録、Retired記録の長期整理は詳細を未決とし、v1で任意資源の取込みや完全削除を追加する理由にはしない。Ubuntuは26.04のx86_64（64bit）のみ、macOSは15以上のARM64で確定。Windows最低版は環境要件の残件であり、OS版はドメイン識別の規則には入れない。
