...
- [PR の通常 CI](https://github.com/wix-diesel/ComposeNest/actions/runs/35845365426) は全 7 ジョブ成功。macOS 15 ARM64 では昇格した管理ルート権限テストと Host Adapter テストを実行した。
- [3 OS のビルド確認](https://github.com/wix-diesel/ComposeNest/actions/runs/35845369151) は macOS 15 ARM64、Windows x86_64、Ubuntu x86_64 ですべて成功。各環境でフロントエンド build と Rust workspace の `cargo check` を実行した。Docker Desktop を含む GUI 実行試験ではない。
