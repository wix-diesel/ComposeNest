...
| ACL 初期化 | 一時的な日本語・空白パスで `Test-ManagementRoot.ps1` 成功。実際の ProgramData への昇格セットアップと GUI からの書込みは未実施 |
| Docker Desktop bind | CLI／Engine 29.5.3、Compose v5.1.4。`alpine:latest` から日本語・空白を含むユーザーの一時パスへ書込み、ホストで `bind-ok` を確認。ProgramData 管理ルート内の bind は未実施 |
...
管理ルート内 bind、通常 GUI からの管理ファイル操作、Windows 11 24H2 は、昇格セットアップと対象環境での追加受入が必要。Docker CLI Adapter は現時点で未実装のため、アプリからの CLI 停止・再照合の試験はその実装時に行う。
