#!/usr/bin/env bash
# Initialize the macOS management root for one explicitly selected user.
set -euo pipefail

readonly MANAGEMENT_ROOT='/Library/Application Support/ComposeNest'
...
check_directory() {
  ...
  read -r actual_uid actual_gid actual_mode < <(stat -f '%u %g %Lp' "$path") || return 1
...
initialize_root() {
  ...
  local uid gid group directory
  uid=$(id -u "$username") || return 1
  gid=$(id -g "$username") || return 1
  group=$(id -gn "$username") || return 1
  if [[ $uid == 0 ]]; then
  ...
  umask 077
  create_directory "$root" "$username" "$group" "$uid" "$gid" || return 1
  for directory in "${DIRECTORIES[@]}"; do
    create_directory "$root/$directory" "$username" "$group" "$uid" "$gid" || return 1
  done
}

if [[ ${BASH_SOURCE[0]} == "$0" ]]; then
  [[ $# == 1 ]] || usage
  [[ $(id -u) == 0 ]] || { echo 'Run initial setup as root.' >&2; exit 1; }
